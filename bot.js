...
const AUTH_TIMEOUT_MS = Number(process.env.AUTH_TIMEOUT_MS || 25000);
...
const botConfigs = {
  1: {
    ...
    restart: Number(process.env.BOT1_RESTART_MINUTES || 10),
  ...
  2: {
    ...
    restart: Number(process.env.BOT2_RESTART_MINUTES || 30),
  ...
  3: {
    ...
    restart: Number(process.env.BOT3_RESTART_MINUTES || 35),
  ...
  4: {
    ...
    restart: Number(process.env.BOT4_RESTART_MINUTES || 25),
...
function authMessage(c, message) {
  ...
  if (!c.authSent && (
    ...
    setTimeout(() => c.authSent = false, 3000);
    return;
  }
...
/* ---------- Easy //b1 / //b2 bot commands ---------- */

function botCommandHelp() {
  return [
    "//b1 start",
    "//b1 stop",
    "//b1 restart",
    "//b1 status",
    "//b1 name NewName",
    "//b1 password NewPassword",
    "//b1 server lobby",
    "//b1 say Hello",
    "//b1 cmd server survival",
    "//b1 help",
  ];
}
...
async function handleBotCommand(input) {
  ...
  if (!parts[0]) return { ok: false, error: "Empty command" };

  const match = parts[0].match(/^\/\/b([1-4])$/i);
  if (!match) {
    return {
      ok: false,
      error: "Use //b1, //b2, //b3 or //b4. Try //b1 help."
    };
  }
  ...
  if (!c) return { ok: false, error: "Bot not found" };

  if (action === "help") {
    return { ok: true, message: botCommandHelp().join("\n") };
  }

  if (action === "start") {
    c.manualStop = false;
    if (!c.bot) connectBot(c);
    return { ok: true, message: `Bot ${id} (${c.name}) starting` };
  }

  if (action === "stop") {
    stopBot(c);
    return { ok: true, message: `Bot ${id} (${c.name}) stopped` };
  }
  ...
  if (action === "restart") {
    ...
    return { ok: true, message: `Bot ${id} (${c.name}) restarting` };
  }

  if (action === "status") {
    return { ok: true, bot: status(c) };
  }

  if (action === "name") {
    if (!rest) return { ok: false, error: "Usage: //b1 name NewName" };
    if (c.bot) {
      return {
        ok: false,
        error: "Stop the bot first, then change its name and start it again."
      };
    }
    c.name = rest.split(/\s+/)[0].slice(0, 16);
    log(c, `name changed to ${c.name} (runtime only)`);
    return { ok: true, message: `Bot ${id} name = ${c.name}` };
  }

  if (action === "password") {
    if (!rest) return { ok: false, error: "Usage: //b1 password NewPassword" };
    if (rest.length < 4) return { ok: false, error: "Password must be at least 4 characters." };
    c.password = rest;
    log(c, "password changed (runtime only)");
    return { ok: true, message: `Bot ${id} password changed` };
  }

  if (action === "server") {
    if (!rest) return { ok: false, error: "Usage: //b1 server lobby" };
    c.server = rest.split(/\s+/)[0];
    log(c, `target server changed to ${c.server} (runtime only)`);
    if (c.bot && c.authenticated) c.bot.chat(`/server ${c.server}`);
    return { ok: true, message: `Bot ${id} target server = ${c.server}` };
  }

  if (action === "say") {
    if (!rest) return { ok: false, error: "Usage: //b1 say Hello" };
    if (!c.bot) return { ok: false, error: "Bot is offline" };
    c.bot.chat(rest);
    log(c, `YOU CHAT -> ${rest}`);
    return { ok: true, message: `Bot ${id} sent chat` };
  }

  if (action === "cmd" || action === "command") {
    if (!rest) return { ok: false, error: "Usage: //b1 cmd server survival" };
    if (!c.bot) return { ok: false, error: "Bot is offline" };
    const msg = rest.startsWith("/") ? rest : `/${rest}`;
    c.bot.chat(msg);
    log(c, `YOU -> ${msg}`);
    return { ok: true, message: `Bot ${id} sent ${msg}` };
  }

  return { ok: false, error: `Unknown action "${action}". Try //b${id} help.` };
}

app.post("/api/bot-command", authorized, async (req, res) => {
  const result = await handleBotCommand(req.body?.command);
...
<div id="panel" style="display:none">
  ...
  <div class="card">
    <b>⚡ Easy Bot Commands</b>
    <div class="small">Use //b1, //b2, //b3 or //b4. Example: //b1 restart</div>
    <select id="botCommandSelect" style="padding:9px;margin:8px 0">
      <option value="1">//b1 — Lobby</option>
      <option value="2">//b2 — Survival</option>
      <option value="3">//b3 — MiniGamers</option>
      <option value="4">//b4 — OneBlock</option>
    </select>
    <select id="botAction" style="padding:9px;margin:8px 0">
    ...
    <input id="botCommandArg" placeholder="Argument (e.g. NewName, lobby, or Hello)">
    <button onclick="sendBotCommand()">Send Bot Command</button>
    <div class="small" style="margin-top:8px">
      Examples: //b1 start · //b2 stop · //b3 restart · //b1 name LobbyBot · //b1 password MyPass123 · //b2 server survival
    </div>
  </div>
...
function startPolling(){
  ...
  pollingStarted = true;
  load();
  statusTimer = setInterval(load, 5000);
  logsTimer = setInterval(loadLogs, 1500);
}

async function saveToken(){
  const input = document.getElementById("token").value.trim();
  if (!input) {
    showLogin("Please enter your control token first.");
    return;
  }
  ...
  try {
    const data = await api("/api/status", { skipAuthFailureReload: true });
    if (!data || !Array.isArray(data.bots)) {
      token = "";
      localStorage.removeItem("gp_token");
      showLogin("Invalid control token.");
      return;
    }
    ...
    token = "";
    localStorage.removeItem("gp_token");
    showLogin("Could not connect to the bot panel.");
  }
}
...
async function api(url, options={}){
  const skipAuthFailureReload = options.skipAuthFailureReload === true;
  delete options.skipAuthFailureReload;
...
async function sendBotCommand(){
  const id = document.getElementById("botCommandSelect").value;
  const action = document.getElementById("botAction").value;
  const arg = document.getElementById("botCommandArg").value.trim();

  let command = "//b" + id + " " + action;
  if (arg) command += " " + arg;

  const data = await api("/api/bot-command", {
    method:"POST",
  ...
  document.getElementById("botCommandArg").value="";
  setTimeout(load,300);
